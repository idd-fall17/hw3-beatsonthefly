...
LSM303Soft::LSM303Soft(void)
{
  ...
  */
  m_min = (LSM303Soft::vector<int16_t>){-32767, -32767, -32767};
  m_max = (LSM303Soft::vector<int16_t>){+32767, +32767, +32767};
...
bool LSM303Soft::timeoutOccurred()
{
...
void LSM303Soft::setTimeout(unsigned int timeout)
{
  io_timeout = timeout;
}

unsigned int LSM303Soft::getTimeout()
{
  return io_timeout;
}

bool LSM303Soft::init(deviceType device, sa0State sa0)
{
...
*/
void LSM303Soft::enableDefault(void)
{
...
void LSM303Soft::writeAccReg(byte reg, byte value)
{
  SoftWire.beginTransmission(acc_address);
  SoftWire.write(reg);
...
byte LSM303Soft::readAccReg(byte reg)
{
  byte value;
  ...
  SoftWire.write(reg);
  last_status = SoftWire.endTransmission();
  SoftWire.requestFrom(acc_address, (byte)1);
  value = SoftWire.read();
...
void LSM303Soft::writeMagReg(byte reg, byte value)
{
  SoftWire.beginTransmission(mag_address);
  SoftWire.write(reg);
  SoftWire.write(value);
  last_status = SoftWire.endTransmission();
}
...
byte LSM303Soft::readMagReg(int reg)
{
  byte value;
  ...
  SoftWire.write(reg);
  last_status = SoftWire.endTransmission();
  SoftWire.requestFrom(mag_address, (byte)1);
  value = SoftWire.read();
  SoftWire.endTransmission();
...
void LSM303Soft::writeReg(byte reg, byte value)
{
...
byte LSM303Soft::readReg(int reg)
{
  // mag address == acc_address for LSM303D, so it doesn't really matter which one we use.
...
void LSM303Soft::readAcc(void)
{
  SoftWire.beginTransmission(acc_address);
  // assert the MSB of the address to get the accelerometer
  // to do slave-transmit subaddress updating.
  SoftWire.write(OUT_X_L_A | (1 << 7));
  last_status = SoftWire.endTransmission();
  SoftWire.requestFrom(acc_address, (byte)6);
  ...
  byte xla = SoftWire.read();
  byte xha = SoftWire.read();
  byte yla = SoftWire.read();
  byte yha = SoftWire.read();
  byte zla = SoftWire.read();
  byte zha = SoftWire.read();
...
void LSM303Soft::readMag(void)
{
  SoftWire.beginTransmission(mag_address);
  // If LSM303D, assert MSB to enable subaddress updating
  // OUT_X_L_M comes first on D, OUT_X_H_M on others
  SoftWire.write((_device == device_D) ? translated_regs[-OUT_X_L_M] | (1 << 7) : translated_regs[-OUT_X_H_M]);
  last_status = SoftWire.endTransmission();
  SoftWire.requestFrom(mag_address, (byte)6);
...
void LSM303Soft::vector_normalize(vector<float> *a)
{
...
int LSM303Soft::testReg(byte address, regAddr reg)
{
