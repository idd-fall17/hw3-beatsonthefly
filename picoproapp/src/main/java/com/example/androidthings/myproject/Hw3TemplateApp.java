...
public class Hw3TemplateApp extends SimplePicoPro {
    ...
    float ax,ay,az; //store analog readings from ADS1015 ADC here [units: V]
    ...
    public void loop() {
        ...
        try {
            ...
            float dx,dy,dz;
            dx = Math.abs(xyz[0]-ax);
            dy = Math.abs(xyz[1]-ay);
            dz = Math.abs(xyz[2]-az);

            if(dx>1.5 && dx>dy && dx>dz) {
                println("X");
                delay(300);
            }
            else if(dy>1.5 && dy>dx && dy>dz) {
                println("Y");
                delay(300);
            }
            else if(dz>1.5 && dz>dx && dz>dy) {
                println("Z");
                delay(300);
            }else {
                ax = .5f * ax + .5f * xyz[0];
